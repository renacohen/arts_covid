...
fit_1 <- lm(sqrt(lost_revenue_total)~ lost_attendees + date, data = small) 
...
lm_wfl_medium <- workflow() %>% 
  add_model(linear_reg() %>%
            set_engine("lm") %>%
            set_mode("regression")) %>% 
  add_recipe(recipe(lost_revenue_total ~  lost_attendees + purpose + date,
                    data = small_train) %>%
...
model_final %>%
  ...
  model_final_tbl <- tibble("Parameter" = c("Intercept", "lost_attendees", "sigma"),
