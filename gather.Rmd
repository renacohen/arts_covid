...
afta_data_clean <- afta_data_new %>%
  ...
  mutate(reduced_philanthropy = ifelse(is.na(`Reduced philanthropic giving:Which of the following 
                                              are currently major financial concerns for your organization? 
                                                (Check all that apply.)`), F, T)) %>%
...
View(afta_covid)
afta_covid <- afta_covid %>%
  mutate(region = case_when())

?case_when

data2 <- afta_covid %>%
        reorder(x = budget, X = c("No Budget/All Volunteer", "Less than 100,000", "100,000 to 249,000",
                                  "250,000 to 499,000", "500,000 to 999,999",
                                  "1,000,000 to 4,999,999", "5,000,000 to 9,999,999", 
                                  "10,000,000 or more"))
