...
afta_covid_2$likelihood_staff_reductions


covid_rates <- inner_join(afta_covid_2, state_pop, by = "state") %>%
  filter(date < "2020-05-14") %>%
  select(state, positive, severity_financial_impact, Pop, likelihood_staff_reductions) %>%
  mutate(pos_rate = positive/Pop) %>%
  group_by(state) %>%
  summarise(financial_severity = mean(severity_financial_impact, na.rm = T),
            pos_rate = mean(pos_rate), 
            likelihood_staff_reductions = mean(likelihood_staff_reductions, na.rm = T), .groups = "drop")
